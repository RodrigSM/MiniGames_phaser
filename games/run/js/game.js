...
Game.Load.prototype = {
  preload: function() {
    ...
    label = game.add.text(w / 2, h / 2, 'a carregar...', {
      font: '30px Arial',
      fill: '#fff'
    });
...
Game.Over.prototype = {
  create: function() {
    label = game.add.text(w / 2, h / 2, 'fim de jogo\n\npontuação: ' + score + '\n\npressiona a tecla SETA PARA CIMA\npara recomeçar', {
      font: '30px Arial',
      fill: '#fff',
      align: 'center'
    });
...
Game.Play.prototype = {
  ...
  create: function() {
    ...
    this.labelScore = game.add.text(15, 10, 'pontuação: 0', {
      font: '20px Arial',
      fill: '#fff'
    });

    this.labelKeys = game.add.text(Math.floor(w / 2) + 1, h - 50, 'usa as setas para te moveres', {
      font: '20px Arial',
      fill: '#fff'
    });
  ...
  update: function() {
    ...
    if (this.game.time.now > this.scoreTime) {
      ...
      score += 1;
      this.labelScore.text = 'pontuação: ' + score;
    }
